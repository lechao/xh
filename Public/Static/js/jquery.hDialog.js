...
;(function($, window, document, undefined) {	
	...
	methods = {
           	...
           	return this.each(function() {
                ...
                if(typeof(opt) == 'undefined') {
                	var defaults = {
						...
						effect: 'fadeOut',         //弹框关闭效果(默认hide，淡出关闭：fadeOut)
        ...
        add: function (o,$this) { //显示弹框
			...
			$body.stop().append("<div id='HOverlay' style='width:"+_doc.width()+"px;height:"+_doc.height()+"px;background-color:"+modalBg+";'></div>");				
			if(o.title != ''){ $obj.stop().prepend('<div id="HTitle">'+o.title+'</div>'); }
			if(o.closeHide != false){
				$obj.stop().append('<a id="HCloseBtn" style="background-color:'+closeBg+';"><span>×</span></a>').css({'position':'fixed','backgroundColor':o.boxBg,'top':t,'left':l,'margin':m,'zIndex':'100'});
			}
        ...
        close: function (o, urls) { //关闭弹框
			...
			switch(o.effect){
				case "hide":
					$obj.stop().hide('slow', _effect);
				break;
				case "fadeOut":
					$obj.stop().fadeOut('slow', _effect);
				break;
				default:
					$obj.stop().fadeOut('slow', _effect);
			}
