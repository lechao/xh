...
<div class="clear"></div>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table1">
        <tr>
            ...
            <td><input id="field_name" type="text" class="input" size="20" name="field[name]" value="<?php echo ($field['name']); ?>" /> <a href="javascript:void(0)" id="check_field_name">检测是否可用</a>（用在数据表中作为字段名，只能由"_"、a~z、A-Z组成）</td>
        </tr>
        <tr>
            <th width="100">字段标签：</th>
            <td><input id="field_label" type="text" class="input" size="20" name="field[comment]" value="<?php echo ($field['comment']); ?>" /> <a href="javascript:void(0)" id="check_field_label">检测是否可用</a>（用在添加数据时，显示在表单前的标签信息）</td>
        </tr>
        <tr>
            <th width="100">字段备注：</th>
            <td><input type="text" class="input" size="29" name="input[remark]" value="<?php echo ($field['input']['remark']); ?>" />（用于描述字段域的意义或填写规则）</td>
        </tr>
        <tr>
        ...
        <tr id="field_length">
            ...
            <td>
                <input type="text" class="input" size="20" name="field[length][intchar]" value="<?php echo ($field['length']['intchar']); ?>" />（字段长度为整数，CHAR、VARCHAR长度不能为空）
            </td>
        </tr>
        <tr id="field_precision">
            <th width="100">字段精度：</th>
            <td>
                <label for="pre_length">长度：</label><input type="text" class="input" size="20" name="field[length][real]" id="pre_length" value="<?php echo ($field['length']['real']); ?>" />&nbsp;&nbsp;&nbsp;<label for="precision">保留位数：</label><input type="text" class="input" size="20" name="field[precision]" id="precision" value="<?php echo ($field['precision']); ?>" />（字段长度、保留位数均为整数）
            </td>
        </tr>
        <tr id="field_value">
            <th width="100">字段默认值：</th>
            <td>
                <input type="text" class="input" size="20" name="field[value]" value="<?php echo ($field['value']); ?>" />
            </td>
        </tr>
     </table>
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table1">
        <tr class="untext_index">
            ...
            <td>
                <label><input type="radio" name="field[is_index]" value="0" <?php if($field['is_index'] == 0): ?>checked="checked"<?php endif; ?> /> 否</label> &nbsp; <label><input type="radio" name="field[is_index]" value="1" <?php if($field['is_index'] == 1): ?>checked="checked"<?php endif; ?>/> 是</label>
            </td>
        </tr>
        <tr class="untext_index">
            <th width="100">是否唯一：</th>
            <td>
                <label><input type="radio" name="field[is_unique]" value="0" <?php if($field['is_unique'] == 0): ?>checked="checked"<?php endif; ?> /> 否</label> &nbsp; <label><input type="radio" name="field[is_unique]" value="1" <?php if($field['is_unique'] == 1): ?>checked="checked"<?php endif; ?> /> 是</label>
            </td>
        </tr>
        <tr>
            <th width="100">是否必填：</th>
            <td>
                <label><input type="radio" name="field[is_require]" value="0" <?php if($field['is_require'] == 0): ?>checked="checked"<?php endif; ?>/> 否</label> &nbsp; <label><input type="radio" name="field[is_require]" value="1" <?php if($field['is_require'] == 1): ?>checked="checked"<?php endif; ?> /> 是</label>
            </td>
        </tr>
        <tr>
            <th width="100">列表中显示：</th>
            <td>
                <label><input type="radio" name="field[is_list_show]" value="0" <?php if($field['is_list_show'] == 0): ?>checked="checked"<?php endif; ?> /> 否</label> &nbsp; <label><input type="radio" name="field[is_list_show]" value="1" <?php if($field['is_list_show'] == 1): ?>checked="checked"<?php endif; ?> /> 是</label> （设置成"是"，则将会在模型数据管理列表中显示出该字段的数据）
            </td>
        </tr>
        <tr>
            <th width="100">自动过滤函数：</th>
            <td>
                <select name="field[auto_filter]" id="auto_filter" width="180">
        ...
        <tr>
            ...
            <td>
                <select name="field[auto_fill]" id="auto_fill" width="180">
                ...
                <select name="field[fill_time]" id="fill_time">
    ...
    <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table1" id="form_opts_tbl">
        <tr id="td_is_show">
            ...
            <td>
               <label><input type="radio" name="input[is_show]" value="0" class="input_is_show" /> 否</label> &nbsp; <label><input type="radio" name="input[is_show]" value="1" class="input_is_show" /> 是</label>
            </td>
        </tr>
        <tr>
        ...
        <tr class="input_text input_password">
            ...
            <td>
                <input type="text" class="input" size="20" name="input[width][text_pwd]" placeholder="默认size为20" <?php if($input['width'] != 0): ?>value="<?php echo ($input['width']); ?>"<?php endif; ?> />
            </td>
        </tr>
        <!-- 单行文本框(text)的内容 -->
        <tr class="input_text">
            ...
            <td>
                <input type="text" class="input" size="20" name="input[value]" value="<?php echo ($input['value']); ?>" />
            </td>
        </tr>
        ...
        <tr class="input_select input_radio input_checkbox">
            ...
            <td><textarea id="content" class="jq_watermark" rows = "5" cols="68" name="input[opt_value]" placeholder="表单域值:显示文本:default（每行一个选项）<br/>1:apple<br/>2:banana:default<br/>3:orange<br/>"><?php echo ($input['opt_value']); ?></textarea></td>
        </tr>
        ...
        <tr class="input_textarea">
            ...
            <td>
                <label for="textarea_width">宽度：</label><input type="text" class="input" size="20" name="input[width][textarea]" id="textarea_width" placeholder="cols" <?php if($input['width'] != 0): ?>value="<?php echo ($input['width']); ?>"<?php endif; ?> />&nbsp;&nbsp;&nbsp;<label for="textarea_height">高度：</label><input type="text" class="input" size="20" name="input[height][textarea]" id="textarea_height" placeholder="rows" <?php if($input['height'] != 0): ?>value="<?php echo ($input['height']); ?>"<?php endif; ?> />
            </td>
        </tr>
        ...
        <tr class="input_editor">
            ...
            <td>
                <label for="editor_width">宽度：</label><input type="text" class="input" size="20" name="input[width][editor]" id="editor_width" placeholder="默认cols为68" <?php if($input['width'] != 0): ?>value="<?php echo ($input['width']); ?>"<?php endif; ?> />&nbsp;&nbsp;&nbsp;<label for="editor_height">高度：</label><input type="text" class="input" size="20" name="input[height][editor]"  id="editor_height" placeholder="默认rows为20" <?php if($input['height'] != 0): ?>value="<?php echo ($input['height']); ?>"<?php endif; ?> />
            </td>
        </tr>
        <tr class="input_editor">
            <th width="100">编辑器类型：</th>
            <td>
               <label><input type="radio" name="input[editor]" value="all" <?php if($input['editor'] == 'all'): ?>checked="checked"<?php endif; ?> /> 全功能型</label> &nbsp; <label><input type="radio" name="input[editor]" value="simple" <?php if($input['editor'] == 'simple'): ?>checked="checked"<?php endif; ?> /> 简洁型</label>
            </td>
        </tr>
        ...
        <tr class="input_file">
            ...
            <td>
                <input type="text" class="input" size="20" name="input[width][upload]" <?php if($input['width'] != 0): ?>value="<?php echo ($input['width']); ?>"<?php endif; ?> />（可以限制上传文件的大小，单位Mb）
            </td>
        </tr>
        ...
        <tr>
            ...
            <td><textarea id="html" rows="4" cols="68" name="input[html]" placeholder="请输入正确的表单域HTML代码，留空则系统自动成"><?php echo htmlspecialchars($input['html']);?></textarea></td>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td>
                <input type="text" class="input" size="20" name="input[show_order]" value="<?php echo ($input['show_order']); ?>"/>（越小表单域位置显示越前，必需大于0）
            </td>
        </tr>
    </table>
    <input type="hidden" name="input[field_id]" value="<?php echo ($field['id']); ?>" />
    <input type="hidden" name="input[id]" value="<?php echo ($input['id']); ?>" />
